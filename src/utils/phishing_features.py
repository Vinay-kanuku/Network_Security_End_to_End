...
class PhishingFeatureExtractor:
    ...
    def __init__(self, use_api=True, verify_ssl=True, cache_ttl=3600):
        ...
        self.shortening_services = [
            ...
            'shorte.st', 'ow.ly', 'buff.ly', 'tiny.cc', 'lnkd.in'
        ]
    ...
    async def extract_features(self, url):
        ...
        async with self:
            ...
            for result, feature_names in [
                ...
                (results[1], ["domain_registeration_length", "age_of_domain", "dnsrecord"]),
                (results[2], [
    ...
    def _extract_url_features(self, url, parsed_url, domain, subdomain):
        ...
        features["having_ip_address"] = 1 if re.match(ip_pattern, parsed_url.netloc) or re.match(ipv6_pattern, parsed_url.netloc) else -1
        ...
        features["shortining_service"] = 1 if any(service in domain for service in self.shortening_services) else -1
        ...
        # 7. having_sub_domain
        features["having_sub_domain"] = -1 if not subdomain else 0 if subdomain.count('.') == 0 else 1
    ...
    async def _check_domain_info(self, domain):
        ...
        if cache_key in self.cache:
            ...
            try:
                domain_info = await asyncio.get_event_loop().run_in_executor(self.executor, lambda: whois.whois(domain))
        ...
        # DNS record
        try:
            await asyncio.get_event_loop().run_in_executor(self.executor, lambda: dns.resolver.resolve(domain, 'A'))
        ...
        try:
            async with self.session.get(url, ssl=self.verify_ssl) as response:
                ...
                features.update({
                    ...
                    "port": 1 if parsed_url.port is None or parsed_url.port in [80, 443, 21, 22] else -1,
                    "https_token": 1 if 'https' in domain else -1,
                    "request_url": self._analyze_external_resources(soup, domain, 'img', 'src'),
                    "url_of_anchor": self._analyze_anchor_tags(soup, domain),
                    "links_in_tags": self._analyze_external_resources_in_tags(soup, domain),
                    "sfh": self._analyze_form_actions(soup, domain),
                    "submitting_to_email": self._check_email_submission(soup),
                    "redirect": -1 if len(response.history) == 0 else 0 if len(response.history) == 1 else 1,
                    "on_mouseover": 1 if 'onmouseover' in html.lower() and ('window.location' in html.lower() or 'window.status' in html.lower()) else -1,
                    "rightclick": 1 if any(p in html.lower() for p in ['event.button==2', 'oncontextmenu="return false"', 'oncontextmenu=']) else -1,
                    "popupwidnow": 1 if any(p in html.lower() for p in ['window.open(', 'alert(', 'prompt(']) else -1,
    ...
    def _analyze_external_resources(self, soup, domain, tag_name, attr_name):
        ...
        if not tags:
            return -1
        external = sum(1 for tag in tags if tag.get(attr_name, '').startswith('http') and tldextract.extract(tag[attr_name]).registered_domain != domain)
    ...
    def _analyze_anchor_tags(self, soup, domain):
        ...
        if not a_tags:
            return -1
        suspicious = sum(1 for a in a_tags if a.get('href', '') in ['#', 'javascript:void(0)'] or 'javascript:' in a.get('href', '') or (a.get('href', '').startswith('http') and tldextract.extract(a['href']).registered_domain != domain))
    ...
    def _analyze_external_resources_in_tags(self, soup, domain):
        ...
        if not tags:
            return -1
        suspicious = sum(1 for tag in tags if (tag.get('src', '') or tag.get('href', '')).startswith('http') and tldextract.extract(tag.get('src') or tag.get('href')).registered_domain != domain)
    ...
    def _analyze_form_actions(self, soup, domain):
        ...
        for form in forms:
            ...
            if action in ['', 'about:blank']:
                return 0
            if action.startswith('http') and tldextract.extract(action).registered_domain != domain:
                return 1
        return -1
    
    def _check_email_submission(self, soup):
        """Check for email submission"""
        if soup.select('a[href^="mailto:"]') or any('mailto:' in form.get('action', '') for form in soup.find_all('form')) or any('mail(' in script.text for script in soup.find_all('script') if script.text):
            return 1
        return -1
