...
class DataValidation:
    ...
    def generate_drift_report(self, base_df: pd.DataFrame, cur_df: pd.DataFrame, threshold: float = 0.05) -> Dict[str, Dict[str, Union[float, bool]]]:
        """
        ...
        for column in base_df.columns:
            if column in cur_df.columns:
                ...
                if not pd.api.types.is_numeric_dtype(d1) or not pd.api.types.is_numeric_dtype(d2):
                    continue
    ...
    def validate_data(self) -> Dict[str, Dict[str, Union[float, bool]]]:
        ...
        try:
            ...
            if not status:
                ...
                raise NetworkException(f"Schema validation failed for training data {e}")
            ...
            base_df = self.read_data(self.data_ingestion_artifact.train_file_path)  # Fixed typo
            ...
            if not drift_report:
                logger.error("Drift report generation failed or no numeric columns available for drift detection.")
